...
//Return how many method to split input string and get two substring which composed by balanced parentheses.
// Character in substring can switch position and '?' can be treat as any kind of parenthes.
//Example: Input: "]()[?[]]"
//Should return 1
//Because there is only one way to split input at index 4, first substring "]()[" can be transformed to balanced parentheses "()[]" by switch position.
// And second substring "?[]]" can be transformed to balanced parentheses "[[]]" by replace '?' with '['.
public class SplitParentheses {
