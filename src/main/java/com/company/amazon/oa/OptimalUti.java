        ...
        while (aInd < a.length && bInd >= 0) {
            ...
            if (sum > target) {
                ...
                if (sum >= max) {
                    ...
                    res.add(new ArrayList<>(Arrays.asList(a[aInd][0], b[bInd][0])));
                    int index = bInd - 1;
                    while (index >= 0 && b[index][1] == b[index + 1][1]) {
                        res.add(new ArrayList<>(Arrays.asList(a[aInd][0], b[index][0])));
                        index--;
                    }
                }
                aInd++;
